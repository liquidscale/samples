...
function LQSContextProvider({ children, name, url }) {
  const [state, dispatch] = useImmerReducer(reducer, {
    local: JSON.parse(localStorage.getItem(name || "lqs_app")) || {},
    ui: {
      loading: false,
      success: false,
      error: false,
  ...
  useEffect(() => {
    if (!state.local.id) {
      ...
      localStorage.setItem(name || "lqs_app", JSON.stringify(local));
    } else {
      localStorage.setItem(name || "lqs_app", JSON.stringify(state.local));
    }
  }, [state.local]);
